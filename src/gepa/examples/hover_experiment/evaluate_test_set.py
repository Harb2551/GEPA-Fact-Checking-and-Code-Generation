...
# =====================
RUN_DIR = "./results/gepa_hover_results_20251107_193824"
TEST_SIZE = 10
TASK_LM = "gpt-4o-mini"
MAX_WORKERS = 2
BATCH_SIZE = 50
...
class TestEvaluationOrchestrator:
    ...
    def _load_test_data(self) -> List[Dict]:
        ...
        test_examples = [self.data_formatter.hover_to_gepa_format(ex) for ex in test_data]
    ...
    def _load_prompts(self) -> Tuple[Dict, Dict]:
        ...
        if not seed_prompt_file.exists():
            ...
            seed_prompt = self.prompt_loader.extract_seed_from_config(config_file, seed_prompt_file)
            if not seed_prompt:
                raise FileNotFoundError(f"Could not find or extract seed prompt")
        else:
            seed_prompt = self.prompt_loader.load_from_file(seed_prompt_file)
        
        # Load optimized prompt
        if not optimized_prompt_file.exists():
            raise FileNotFoundError(f"Optimized prompt not found at {optimized_prompt_file}")
        
        optimized_prompt = self.prompt_loader.load_from_file(optimized_prompt_file)
